<?php
    class widget_custom_post extends WP_Widget {
        ...
        function widget_custom_post() {
            $widget_ops = array( 'classname' => 'widget_custom_post' , 'description' => __( " Latest custom posts" , 'redcodn' ) );
            $this -> WP_Widget( 'widget_red_custom_post' , _TN_ . ': ' . __( " Latest custom posts" , 'redcodn' ) , $widget_ops );
        }
            ...
            if( is_array( $recent -> posts ) && !empty( $recent -> posts ) ){
                ...
                foreach( $recent -> posts as $post )  {
					if( get_post_thumbnail_id( $post -> ID ) ){
								...
								$post_img = '<img src="' . get_template_directory_uri() . '/images/no.image.' . image::tsize( 'thumbnail' ) . '.png" alt="" />';
								$cnt_a1 = ' href="' . get_permalink($post -> ID) . '"';
								$cnt_a2 = ' href="' . get_permalink($post -> ID) . '#comments"';
								$cnt_a3 = ' class="entry-img" href="' . get_permalink($post -> ID) . '"';
							}
...
// 			print_r($old_instance);
    ...
    ?>
			...
			<div>
				<p>
					<label ><?php _e('Select post type','redcodn') ?>: 
						<!-- <a href="javascript:void(0)" onclick="jQuery(this).parent().parent().parent().remove();" style="float:right"><?php _e("remove",'redcodn'); ?></a> -->
						<select class="widefat post_type" onchange="get_taxonomy(jQuery(this))" name="<?php echo $this->get_field_name( 'customPost'  ); ?>[]" >
							<option value=''  ><?php _e('Select item','redcodn'); ?></option>	
						<?php foreach($post_types as $key => $custom_post) {  
							if('attachment' != $key && "slideshow"!= $key && "page"!= $key){ 
						?>
				...
				<?php 
					$custom_posts_taxonomies = array(
							'post' => array('category' => __('Category','redcodn'), 
											'post_tag' => __('Post tag','redcodn') ,
										),
							
							'gallery' => array('gallery-category' => __('Gallery Category','redcodn'), 
												'gallery-tag' => __('Gallery tag','redcodn') ,
										),
						);
					...
					?>	
						<p style="display:none"> 
							<label ><?php _e('Select post taxonomy','redcodn') ?>: 
								<select class="widefat " style="display:none" name="<?php  echo $this->get_field_name( 'taxonomy'  ); ?>[]" >
									<option value="__"><?php _e('Select taxonomy','redcodn') ?></option>
								</select>
							</label>	
						</p>
				...
				<div class="taxonomies">
					<?php
						...
						?>
						<p> 
							<label ><?php _e('Select post terms','redcodn') ?>: 
			...
			<p>
                <label for="<?php echo $this->get_field_id('nr_posts'); ?>"><?php _e( 'Number of posts' , 'redcodn' ) ?>:
