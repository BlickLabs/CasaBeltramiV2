...
class widget_flickr extends WP_Widget {
    function widget_flickr() {
        ...
        /* Constructor */
        $widget_ops = array('classname' => 'widget_flickr_photos', 'description' => __( 'Flickr Photos' , 'redcodn' ) );
        $this->WP_Widget('widget_red_flickrwidget', _TN_ . ' : ' . __( 'Flickr Photos' , 'redcodn' ), $widget_ops);
    }
    ...
    function widget($args, $instance) {
        ...
        $title = empty($instance['title']) ? __('Photo Gallery','redcodn') : apply_filters('widget_title', $instance['title']);
...
?>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of photos','redcodn') ?>:
                <input class="widefat" id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo esc_attr($number); ?>" />
            </label>
        </p>

        <p>
            <label for="<?php echo $this->get_field_id('showing'); ?>"><?php _e('Showing Method','redcodn') ?>:
                <select size="1" name="<?php echo $this->get_field_name('showing'); ?>">
                    <option value="random"<?php if(esc_attr($showing) =='random'){echo 'selected';}?>><?php _e('Random Photo','redcodn')?></option>
                    <option value="latest"<?php if(esc_attr($showing) =='latest'){echo 'selected';}?>><?php _e('Latest Photo','redcodn') ?></option>
                </select>
            </label>
        </p>
<?php
