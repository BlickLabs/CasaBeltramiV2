...
class widget_instagram extends WP_Widget {
	...
	function widget_instagram() {
        $options = array( 'classname' => 'instagram_widget', 'description' => __('Instagram Widget' , 'redcodn' ) );
        parent::WP_Widget( 'widget_red_instagram' , _TN_ . ' : ' . __( 'Instagram Widget' , 'redcodn' )  , $options );
	...
	function widget( $args, $instance ) {
		...
		echo "<ul id='instagram_widget' class='widget-list ". $layout ."'>";
		if(!empty($results['data'])){
			foreach($results['data'] as $item){
				if($picture_number == 0){

					echo sprintf(__('%s Please set the Number of images to show within the widget %s','redcodn'),'<strong>', '</strong>');
					break;
				}
				...
				if($show_likes){
					if(!empty($item['likes']['count'])){
						echo "<p class='instagram_likes'>".__('Likes:','redcodn')." <span class='likes_count'>".$item['likes']['count']."</span></p>";
					}
				}
				echo '</div>';
				if($show_caption){
			...
			echo "<strong>".__('The user currently does not have any images...','redcodn')."</strong>";			
		}
		echo $after_widget;
	}
	...
	function form( $instance ) {
		if ( $instance ) {
			...
			$title = __( 'Title', 'redcodn' );
			$username = __( 'Username', 'redcodn' );
			$access_token = __( 'Access Token', 'redcodn' );
			$user_id = __( 'User ID', 'redcodn' );
			$picture_size = 'thumbnail';
			$layout = 'normal';
			$picture_number = 0;
			$show_likes = false;
		...
		$redirect_uri = 'http://redcodn.com/instagram/';
		$token_url = 'https://api.instagram.com/oauth/authorize/?client_id='.$client_id.'&redirect_uri='.$redirect_uri.'&response_type=code';
		$get_token_url = 'http://redcodn.com/instagram';

		$picture_sizes = array('thumbnail' => 'Thumbnail', 'low_resolution' => 'Low Resolution','standard_resolution' => 'Standard Resolution');
		$layouts = array('normal' => '2 per row', 'fullwidth' => '20 per row');
		?>
		<p>
		<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:','redcodn'); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
		</p>
		<p>
		<label for="<?php echo $this->get_field_id('user_id'); ?>"><?php _e('User ID:','redcodn'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('access_token'); ?>"><?php _e('Access Token:','redcodn'); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id('picture_number'); ?>"><?php _e('Number of Images:','redcodn'); ?></label> 
			<select id="<?php echo $this->get_field_id('picture_number'); ?>" name="<?php echo $this->get_field_name('picture_number'); ?>">
					<option value="0"><?php _e('Select Number','redcodn'); ?></option>
				<?php for($i=2;$i<21;$i++):?>
		...
		<p>
			<label for="<?php echo $this->get_field_id('picture_size'); ?>"><?php _e('Picture Size:','redcodn'); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id('layout'); ?>"><?php _e('Layout:','redcodn'); ?></label> 
			<select id="<?php echo $this->get_field_id('layout'); ?>" name="<?php echo $this->get_field_name('layout'); ?>">
					<?php foreach($layouts as $item => $val):?>
						<option value="<?php echo $item;?>" <?php if($item == $layout){echo 'selected="selected"';};?>><?php echo $val;?></option>
					<?php endforeach;?>
			</select>
		</p>
		<p>
		<label for="<?php echo $this->get_field_id('link_images'); ?>"><?php _e('Link images to full image:','redcodn'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('show_likes'); ?>"><?php _e('Show Likes:','redcodn'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('show_caption'); ?>"><?php _e('Show Caption:','redcodn'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('debug_mode'); ?>"><?php _e('Debug Mode:','redcodn'); ?></label> 
		...
		<?php
			$token_msg = sprintf(__('If you do not have an ID or access token, please visit %s Get Access token %s to receive a valid token','redcodn'),'<a href="'.$get_token_url.'" target="_blank">', '</a>');
		?>
		<p><?php echo $token_msg; ?></p>
		<p>If you don't know your user id: <a href="http://jelled.com/instagram/lookup-user-id" target="_blank">Get you user id</a></p>
		<?php 
	}
