<?php
    ...
    class tweets{
        ...
        function get_tweets( $username , $number , $classes = '' , $before = "<div class='tweet_item'><p>" , $after = "</p></div>", $static_class = '' ){
        	if(trim($username) == ''){
        		$result[0] = '<div class="red_twitter">';
	  			$result[0] .='	<div class="slides_container">';
	  			$result[0] .= '<div>' . $before . __('Please set Twitter user name !!!' , 'redcodn') .  $after . '</div>';
	  			$result[0] .= '</div>';
	  			$result[0] .= '</div>';

	  			$result[1]  = '<a class="i_join_us '.$static_class.'" href="http://twitter.com/" title="'.__('Follow us','redcodn').'">'.__('Follow us','redcodn').'</a>';
	  			return $result;
        	}
            ...
            if( strlen( $classes ) ){  /*This is for the front page presentation*/
	            ...
	            if( !isset( $tweets_list_num ) || $tweets_list_num == 0) {
	                $result[0] .= '<li>' . $before . __( 'Unable to read tweets !!!' , 'redcodn' ) . $after . '</li>';
	            }else {
	            ...
	            /*If the number of tweets set by the user is 0 or if no tweets are found, the widget will show 'no items' instead.*/
	            if( !isset( $tweets_list_num ) || $tweets_list_num == 0) {
	                $result[0] .= '<div>' . $before . __( 'Unable to read tweets !!!' , 'redcodn' ) . $after . '</div>';
	            }else {
	                /*Otherwise, we will loop through the array and print out each of the tweets individually.*/
	                if( isset( $tweets ) && is_array( $tweets ) && !empty( $tweets ) ){
	                    foreach( $tweets as $tweet ) {
	            ...
	            $result[1]  = '<a class="i_join_us '.$static_class.'" href="http://twitter.com/'.$username.'" title="'.__('Follow us','redcodn').'">'.__('Follow us','redcodn').'</a>';
            }
